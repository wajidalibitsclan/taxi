...
class CarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($list)) {
            $distanceKm = 0;
            if(!empty($gmap_distance  = $request->input('gmap_distance'))){
                $distanceKm = $gmap_distance/1000;
            }
            foreach ($list as $row) {
                $price = $row->price;
                if(!empty($prices = $row->prices)){
                    foreach ($prices as $value){
                        if($value->from <= $distanceKm and $distanceKm<= $value->to){
                            $price = $value->oneway_discount?$value->oneway_discount:$value->oneway_price;
                            break;
                        }
                    }
                }
                ...
                $markers[] = [
                    ...
                    "title"   => $row->title,
                    "price"   => $row->price,
                    "price_html"   => format_money($row->price),
    ...
    public function detail(Request $request, $slug)
    {
       ...
       // dd($request->all());
        ...
        $distanceKm = 0;
        if(!empty($gmap_distance  = $request->query('gmap_distance'))){
            $distanceKm = $gmap_distance/1000;
        }
