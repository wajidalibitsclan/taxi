            ...
            <div class="col-left">
                <form method="get" action="" class="filter-form filter-form-right d-flex justify-content-end">
                    <select name="object_model" class="form-control">
                        ...
                        @foreach(get_bookable_services() as $id=>$model)
                            <?php if(!$model::isEnable()) continue; ?>
                            <option value="{{$id}}" @if($id == request('object_model')) selected @endif>{{ucfirst($id)}}</option>
                        @endforeach
                    </select>
                    <select name="status" class="form-control">
                        <option value="">[Status]</option>
                        @foreach($statues as $status)
                            <option value="{{$status}}" @if($status == request('status')) selected @endif>{{booking_status_to_text($status)}}</option>
                        @endforeach
                    </select>
                    ...
                    <table class="table table-hover bravo-list-item">
                        ...
                        @foreach($rows as $row)
                            ...
                            <tr>
                                ...
                                <td>{{ucfirst($row->object_model)}}</td>
                                <td>
                                    <ul>
                                ...
                                @endphp
                                <td> {{$meta['from_address'] ?? ''}}</td>
                                <td> {{$meta['to_address'] ?? ''}}</td>
                                <td>{{ isset($meta['pickup_date']['date']) ? display_datetime($meta['pickup_date']['date']) : '' }}</td>
                                <td> {{ isset($meta['return_date']['date']) ? display_datetime($meta['return_date']['date']) : '' }}</td>
                            ...
                            @endif
                                ...
                                <td>
                                    ...
                                    {{__("Service Charge")}} : {{format_money($booking->total_before_extra_price)}}<br/>
                                    {{__("Extras")}} : {{$total_extra}}<br/>
                                    {{__("Total")}} : {{format_money_main($row->total)}}<br/>
                                ...
                                <td> {{$meta['pickup_flight'] ?? ''}}</td>
                                <td> {{$meta['passenger'] ?? ''}} | {{$meta['baggage'] ?? ''}}</td>
                                <td>
                                    @if($service = $row->service)
                                        <div class="dropdown">
