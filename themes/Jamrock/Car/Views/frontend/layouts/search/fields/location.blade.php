    ...
    <div class="form-content">
        ...
        @if($location_search_style=='autocompletePlace')
            <div class="g-map-place" >
                <input type="text" id="{{$name ?? ''}}" name="{{ $prefix ?? "" }}map_place" placeholder="{{__("Enter your pickup location?")}}"  value="{{request()->input('map_place')}}" class="form-control border-0 map_place">
                <div class="map d-none" id="map-{{\Illuminate\Support\Str::random(10)}}"></div>
                <input type="hidden" class="map_lat" name="{{ $prefix ?? "" }}map_lat" value="{{request()->input(($prefix ?? "").'map_lat')}}">
                <input type="hidden" class="map_lgn" name="{{ $prefix ?? "" }}map_lgn" value="{{request()->input(($prefix ?? "").'map_lgn')}}">
            </div>
        @else
        <?php
        $location_name = "";
        ...
        <div class="smart-search">
            <input type="text" class="smart-search-location parent_text form-control" {{ ( empty(setting_item("car_location_search_style")) or setting_item("car_location_search_style") == "normal" ) ? "readonly" : ""  }} placeholder="{{ $title_input ?? "" }}" value="{{ $location_name }}" data-onLoad="{{__("Loading...")}}"
                   data-default="{{ json_encode($list_json) }}">
            <input type="hidden" class="child_id" name="{{ $prefix ?? "" }}location_id" value="{{Request::query(($prefix ?? "").'location_id')}}">
        </div>
            @endif
    </div>
</div>
