...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{__("Dropdown Options")}} for {{$dropdown->title}}</h1>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{ route('options.admin.bulkedit') }}" class="filter-form filter-form-left d-flex justify-content-start">
                                {{csrf_field()}}
                    ...
                    <div class="col-left">
                        ...
                        <a href="{{route('options.admin.create', $dropdown->id)}}" class="btn btn-primary">Add new Option</a>
                    </div>
                </div>
                <div class="panel">
                                ...
                                @if( count($rows) > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td class="title">
                                                <a href="{{ route('options.admin.edit', $row->id) }}">{{$row->title}}</a>
                                            </td>
                                            <td>
                                                {{$row->value}}
                                            </td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <a class="dropdown-item" href="{{ route('options.admin.edit', ['id' => $row->id]) }}">Edit</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
